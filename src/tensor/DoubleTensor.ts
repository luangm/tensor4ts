...
export default class DoubleTensor extends TensorBase {
  ...
  constructor(data: Float64Array, shape: number[], strides?: number[], offset: number = 0) {
    super(data, shape, strides, offset);
    this._data = data;
  }

  protected create(data: TensorBufferLike, shape: number[], strides?: number[], offset?: number): Tensor {
    return new DoubleTensor(data as Float64Array, shape, strides, offset);
  }
